...
public partial class CAdsManager : CSingleton<CAdsManager> {
	...
	public struct STParams {
		...
		public string m_oAppLovinSDKKey;
		public Dictionary<string, string> m_oAppLovinAdsIDDict;
...
#if ADMOB_ADS_ENABLE
	private bool m_bIsInitAdmob = false;
	private bool m_bIsLoadAdmobBannerAds = false;

	private AdRequest m_oAdmobRequest = null;
...
#if IRON_SRC_ADS_ENABLE
	private bool m_bIsInitIronSrc = false;
	private bool m_bIsLoadIronSrcBannerAds = false;
#endif // #if IRON_SRC_ADS_ENABLE

#if APP_LOVIN_ADS_ENABLE
	private bool m_bIsInitAppLovin = false;
	private bool m_bIsLoadAppLovinBannerAds = false;
#endif // #if APP_LOVIN_ADS_ENABLE
	#endregion // 변수
	...
	public float OriginBannerAdsHeight { get; private set; } = 0.0f;
	...
	private AdRequest AdmobRequest {
		get {
			...
			// 요청 빌더 생성이 가능 할 경우
			if(this.IsInitAdmob() && m_oAdmobRequest == null) {
				m_oAdmobRequest = new AdRequest();
			}

			CAccess.Assert(m_oAdmobRequest != null);
			return m_oAdmobRequest;
#else
			return null;
...
#if UNITY_IOS || UNITY_ANDROID
			// 배너 공고 생성이 가능 할 경우
			if(m_oAdmobBannerAds == null && this.IsEnableBannerAds && this.IsInitAdmob() && this.IsValidAdmobBannerAdsID()) {
				...
				m_oAdmobBannerAds.OnBannerAdLoaded += this.OnLoadAdmobBannerAds;
				m_oAdmobBannerAds.OnBannerAdLoadFailed += this.OnLoadFailAdmobBannerAds;
			}
...
#if APP_LOVIN_ADS_ENABLE
		m_oFuncDictContainer01[EAdsFunc.LOAD_BANNER_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.LoadAppLovinBannerAds);
		m_oFuncDictContainer01[EAdsFunc.LOAD_REWARD_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.LoadAppLovinRewardAds);
		m_oFuncDictContainer01[EAdsFunc.LOAD_FULLSCREEN_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.LoadAppLovinFullscreenAds);

		m_oFuncDictContainer01[EAdsFunc.SHOW_BANNER_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.ShowAppLovinBannerAds);
		m_oFuncDictContainer01[EAdsFunc.SHOW_REWARD_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.ShowAppLovinRewardAds);
		m_oFuncDictContainer01[EAdsFunc.SHOW_FULLSCREEN_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.ShowAppLovinFullscreenAds);

		m_oFuncDictContainer01[EAdsFunc.CLOSE_BANNER_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.CloseAppLovinBannerAds);

		m_oFuncDictContainer02[EAdsFunc.IS_INIT].TryAdd(EAdsPlatform.APP_LOVIN, this.IsInitAppLovin);
		m_oFuncDictContainer02[EAdsFunc.IS_LOAD_BANNER_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.IsLoadAppLovinBannerAds);
		m_oFuncDictContainer02[EAdsFunc.IS_LOAD_REWARD_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.IsLoadAppLovinRewardAds);
		m_oFuncDictContainer02[EAdsFunc.IS_LOAD_FULLSCREEN_ADS].TryAdd(EAdsPlatform.APP_LOVIN, this.IsLoadAppLovinFullscreenAds);
#endif // #if APP_LOVIN_ADS_ENABLE
	}
...
#if APP_LOVIN_ADS_ENABLE
		CAccess.Assert(a_stParams.m_oAppLovinSDKKey.ExIsValid() && a_stParams.m_oAppLovinAdsIDDict.ExIsValid());
#endif // #if APP_LOVIN_ADS_ENABLE

#if !UNITY_EDITOR && (UNITY_IOS || UNITY_ANDROID)
		// 초기화 되었을 경우
		if(this.IsInit) {
			a_stParams.m_oCallbackDict?.GetValueOrDefault(ECallback.INIT)?.Invoke(this, EAdsPlatform.NONE, this.IsInit);
		} else {
			...
			IronSourceEvents.onRewardedVideoAvailabilityChangedEvent += this.OnLoadIronSrcRewardAds;
			IronSourceEvents.onRewardedVideoAdLoadFailedEvent += this.OnLoadFailIronSrcRewardAds;
			...
			MaxSdkCallbacks.Banner.OnAdLoadedEvent += this.OnLoadAppLovinBannerAds;
			MaxSdkCallbacks.Banner.OnAdLoadFailedEvent += this.OnLoadFailAppLovinBannerAds;

			MaxSdkCallbacks.Rewarded.OnAdLoadedEvent += this.OnLoadAppLovinRewardAds;
			MaxSdkCallbacks.Rewarded.OnAdLoadFailedEvent += this.OnLoadFailAppLovinRewardAds;
			MaxSdkCallbacks.Rewarded.OnAdHiddenEvent += this.OnCloseAppLovinRewardAds;
			MaxSdkCallbacks.Rewarded.OnAdReceivedRewardEvent += this.OnReceiveAppLovinAdsReward;

			MaxSdkCallbacks.Interstitial.OnAdLoadedEvent += this.OnLoadAppLovinFullscreenAds;
			MaxSdkCallbacks.Interstitial.OnAdLoadFailedEvent += this.OnLoadFailAppLovinFullscreenAds;
			MaxSdkCallbacks.Interstitial.OnAdHiddenEvent += this.OnCloseAppLovinFullscreenAds;

			MaxSdk.SetSdkKey(a_stParams.m_oAppLovinSDKKey);
			MaxSdk.InitializeSdk(this.MakeAppLovinAdsIDs(a_stParams.m_oAppLovinAdsIDDict).ToArray());
#endif // #if APP_LOVIN_ADS_ENABLE
		}
#else
	...
	public override void OnLateUpdate(float a_fDeltaTime) {
		...
		// 앱이 실행 중 일 경우
		if(CSceneManager.IsAppRunning) {
			m_fUpdateSkipTime += CScheduleManager.Inst.UnscaleDeltaTime;

			// 광고 로드 주기가 지났을 경우
			if(m_fUpdateSkipTime.ExIsGreatEquals(KCDefine.U_DELTA_T_ADS_M_ADS_LOAD)) {
	...
	public void LoadBannerAds(EAdsPlatform a_eAdsPlatform) {
		...
		// 배너 광고 로드가 가능 할 경우
		if(this.IsEnableBannerAds && this.IsInitAds(a_eAdsPlatform) && !this.IsLoadBannerAds(a_eAdsPlatform)) {
	...
	public void LoadRewardAds(EAdsPlatform a_eAdsPlatform) {
		...
		// 보상 광고 로드가 가능 할 경우
		if(this.IsEnableRewardAds && this.IsInitAds(a_eAdsPlatform) && !this.IsLoadRewardAds(a_eAdsPlatform)) {
	...
	public void LoadFullscreenAds(EAdsPlatform a_eAdsPlatform) {
		...
		// 전면 광고 로드가 가능 할 경우
		if(this.IsEnableFullscreenAds && this.IsInitAds(a_eAdsPlatform) && !this.IsLoadFullscreenAds(a_eAdsPlatform)) {
	...
	private void SetupBannerAdsHeight(float a_fHeight) {
		...
		this.BannerAdsHeight = CAccess.GetBannerAdsHeight(a_fHeight);
		this.OriginBannerAdsHeight = a_fHeight;
	}
...
public partial class CAdsManager : CSingleton<CAdsManager> {
	...
	public bool IsInitAds(EAdsPlatform a_eAdsPlatform) {
		CAccess.Assert(a_eAdsPlatform.ExIsValid());
		return this.IsInit && m_oFuncDictContainer02[EAdsFunc.IS_INIT][a_eAdsPlatform]();
	}

	/** 배너 광고 로드 여부를 검사한다 */
	public bool IsLoadBannerAds(EAdsPlatform a_eAdsPlatform) {
		CAccess.Assert(a_eAdsPlatform.ExIsValid());
		return this.IsEnableBannerAds && this.IsInitAds(a_eAdsPlatform) && m_oFuncDictContainer02[EAdsFunc.IS_LOAD_BANNER_ADS][a_eAdsPlatform]();
	}

	/** 보상 광고 로드 여부를 검사한다 */
	public bool IsLoadRewardAds(EAdsPlatform a_eAdsPlatform) {
		CAccess.Assert(a_eAdsPlatform.ExIsValid());
		return this.IsEnableRewardAds && this.IsInitAds(a_eAdsPlatform) && m_oFuncDictContainer02[EAdsFunc.IS_LOAD_REWARD_ADS][a_eAdsPlatform]();
	}

	/** 전면 광고 로드 여부를 검사한다 */
	public bool IsLoadFullscreenAds(EAdsPlatform a_eAdsPlatform) {
		CAccess.Assert(a_eAdsPlatform.ExIsValid());
		return this.IsEnableFullscreenAds && this.IsInitAds(a_eAdsPlatform) && m_oFuncDictContainer02[EAdsFunc.IS_LOAD_FULLSCREEN_ADS][a_eAdsPlatform]();
	}
...
#if APP_LOVIN_ADS_ENABLE
			m_oAppLovinAdsIDDict = new Dictionary<string, string>()
#endif // #if APP_LOVIN_ADS_ENABLE
		};
	}
	#endregion // 클래스 함수
