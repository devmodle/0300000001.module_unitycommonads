...
public partial class CAdsManager : CSingleton<CAdsManager> {
	...
	private void CloseIronSrcBannerAds() {
		...
		m_bIsLoadIronSrcBannerAds = false;
	...
	private void OnInitIronSrc() {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_INIT_CALLBACK, () => {
			...
			this.IsInit = true;
			m_bIsInitIronSrc = true;

			this.Params.m_oCallbackDict?.GetValueOrDefault(ECallback.INIT)?.Invoke(this, EAdsPlatform.IRON_SRC, m_bIsInitIronSrc);
		});
	}
	...
	private void OnLoadIronSrcBannerAds() {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_BANNER_ADS_LOAD_CALLBACK, () => {
	...
	private void OnLoadFailIronSrcBannerAds(IronSourceError a_oError) {
		CFunc.ShowLog($"CAdsManager.OnLoadFailIronSrcBannerAds: {a_oError?.getErrorCode()}, {a_oError?.getDescription()}", KCDefine.B_LOG_COLOR_PLUGIN);
	...
	private void OnLoadIronSrcRewardAds(bool a_bIsActive) {
		CFunc.ShowLog($"CAdsManager.OnLoadIronSrcRewardAds: {a_bIsActive}", KCDefine.B_LOG_COLOR_PLUGIN);

		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_REWARD_ADS_LOAD_CALLBACK, () => {
			// Do Something
		});
	}
	...
	private void OnLoadFailIronSrcRewardAds(IronSourceError a_oError) {
		CFunc.ShowLog($"CAdsManager.OnLoadIronSrcRewardAds: {a_oError?.getErrorCode()}, {a_oError?.getDescription()}", KCDefine.B_LOG_COLOR_PLUGIN);
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_REWARD_ADS_LOAD_FAIL_CALLBACK, () => this.AddLoadFailRewardAdsInfo(EAdsPlatform.IRON_SRC, this.LoadRewardAds));
	}
	...
	private void OnCloseIronSrcRewardAds() {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_REWARD_ADS_CLOSE_CALLBACK, () => {
			this.HandleCloseRewardAdsResult(EAdsPlatform.IRON_SRC);
			this.LoadRewardAds(EAdsPlatform.IRON_SRC);
		});
	}
	...
	private void OnReceiveIronSrcAdsReward(IronSourcePlacement a_oPlacement) {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_REWARD_ADS_RECEIVE_ADS_REWARD_CALLBACK, () => {
			this.HandleRewardAdsResult(EAdsPlatform.IRON_SRC, (a_oPlacement != null) ? new STAdsRewardInfo() {
				m_oID = a_oPlacement.getRewardName().ExIsValid() ? a_oPlacement.getRewardName() : string.Empty, m_oVal = $"{a_oPlacement.getRewardAmount()}"
	...
	private void OnLoadFailIronSrcFullscreenAds(IronSourceError a_oError) {
		CFunc.ShowLog($"CAdsManager.OnLoadFailIronSrcFullscreenAds: {a_oError?.getErrorCode()}, {a_oError?.getDescription()}", KCDefine.B_LOG_COLOR_PLUGIN);
	...
	private void OnCloseIronSrcFullscreenAds() {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_ADS_M_IRON_SRC_FULLSCREEN_ADS_CLOSE_CALLBACK, () => {
			this.HandleCloseFullscreenAdsResult(EAdsPlatform.IRON_SRC);
			this.LoadFullscreenAds(EAdsPlatform.IRON_SRC);
		});
	}
	...
	/** 광고 식별자를 생성한다 */
	private List<string> MakeIronSrcAdsIDs(Dictionary<string, string> a_oAdsIDDict) {
...
#if(UNITY_IOS || UNITY_ANDROID) && IRON_SRC_ADS_ENABLE
		return this.IsValidIronSrcBannerAdsID() && m_bIsLoadIronSrcBannerAds;
#else
		return false;
#endif // #if (UNITY_IOS || UNITY_ANDROID) && IRON_SRC_ADS_ENABLE
	}
